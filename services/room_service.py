...
class RoomService:
    ...
    async def create_room(self, name: str, host_id: str = None, game_instance=None) -> Tuple[Room, RoomManager]:
        ...
        Returns:
            Tuple[Room, RoomManager]: 创建的房间和房间管理器
        """
    ...
    async def add_player_to_room(self, room_manager: RoomManager, player_id: str, player_name: str) -> Tuple[Player, List[Dict[str, str]]]:
        """添加玩家到房间
        ...
        room_messages = self.broadcast_to_room(room_manager, join_message, [player_id])
        messages.extend(room_messages)
        
        # 通知新加入的玩家
        welcome_message = f"欢迎加入房间 {room_manager.room.name}"
    ...
    async def remove_player_from_room(self, room_manager: RoomManager, player_id: str) -> Tuple[bool, Optional[PlayerLeftEvent], List[Dict[str, str]]]:
        """将玩家从房间中移除
        ...
        """
        # 使用RoomManager移除玩家
        success, event = room_manager.remove_player(player_id)
        ...
        if event:
            ...
            room_messages = self.broadcast_to_room(room_manager, leave_message)
            messages.extend(room_messages)
    ...
    def broadcast_to_room(self, room_manager: RoomManager, message: str, exclude_players: List[str] = None) -> List[Dict[str, str]]:
        """向房间中所有玩家广播消息
    ...
    async def set_player_ready(self, room_manager: RoomManager, player_id: str, is_ready: bool = True) -> Tuple[bool, List[Dict[str, str]]]:
        """设置玩家准备状态
        ...
        """
        # 使用RoomManager设置玩家准备状态
        success = room_manager.set_player_ready(player_id, is_ready)
    ...
    async def kick_player(self, room_manager: RoomManager, host_id: str, player_id: str) -> Tuple[bool, List[Dict[str, str]]]:
        """房主踢出玩家
        ...
        if event:
            ...
            room_messages = self.broadcast_to_room(room_manager, kick_message)
            messages.extend(room_messages)
    ...
    async def select_character(self, room_manager: RoomManager, player_id: str, character_name: str) -> Tuple[bool, str, List[Dict[str, str]]]:
        """玩家选择角色
    ...
    async def set_scenario(self, room_manager: RoomManager, scenario_id: str) -> Tuple[bool, Optional[str], List[Dict[str, str]]]:
        """设置当前游戏使用的剧本
