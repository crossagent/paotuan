...
class DMNarrationCommand(GameCommand):
    ...
    async def execute(self, event: DMNarrationEvent) -> List[Union[GameEvent, Dict[str, str]]]:
        ...
        ai_service = self.service_provider.get_service(AIService)
        ...
        if not await match_service.is_match_running(match_context):
            logger.warning(f"游戏局未在运行中: ID={match_context.match.id}")
            return []
        ...
        # 获取上下文信息
        context = await narration_service.prepare_context(match_context, turn_context, players)
        
        # 加载剧本（如果有）
        scenario = await narration_service.load_scenario(match_context)
        
        # 调用AI服务生成叙述
        response = await ai_service.generate_narration(context, scenario)
        
        # 处理AI响应
        messages = []
        
        # 处理位置、物品和剧情更新
        update_messages = await narration_service.process_ai_response(response, room_context, scenario)
        ...
        for notification in system_notifications:
            if notification.get("action") == "finish_match":
                ...
                await match_service.end_match(match_context, result)
                logger.info(f"游戏局已结束：ID={match_context.match.id}, 结果={result}")
